...
async def run_memory_ablation(trials: int = 3) -> dict[str, object]:
    ...
    def aggregate(enabled: bool) -> dict[str, object]:
        ...
        return {"trials": len(arm), "return_inspection_rate": sum(bool(row["agent_completed_return_inspection"]) for row in arm) / len(arm), "fault_suspicion_rate": sum(bool(row["agent_suspicion"]) for row in fault_rows) / len(fault_rows), "verifier_confirmation_rate": sum(bool(row["confirmed_finding"]) for row in fault_rows) / len(fault_rows), "healthy_false_positive_rate": sum(bool(row["healthy_false_positive"]) for row in healthy_rows) / len(healthy_rows), "mean_context_tokens": round(sum(float(row["context_tokens"]) for row in arm) / len(arm), 2), "mean_model_input_tokens": round(sum(float(row["model_input_tokens"]) for row in arm) / len(arm), 2), "mean_model_output_tokens": round(sum(float(row["model_output_tokens"]) for row in arm) / len(arm), 2), "mean_action_count": round(sum(float(row["action_count"]) for row in arm) / len(arm), 2), "mean_duration_ms": round(sum(float(row["duration_ms"]) for row in arm) / len(arm), 2)}

    arms = {"memory_off": aggregate(False), "memory_on": aggregate(True)}
    return {"evaluation": "deterministic_two_visit_memory_ablation", "trials_per_fault_per_arm": trials, "arms": arms, "deltas_memory_on_minus_off": {key: arms["memory_on"][key] - arms["memory_off"][key] for key in ("return_inspection_rate", "fault_suspicion_rate", "healthy_false_positive_rate", "mean_context_tokens", "mean_action_count", "mean_duration_ms")}, "rows": rows, "limitations": ["Uses a deterministic policy to isolate context-retrieval behavior; it is not a Qwen benchmark.", "Model output tokens are zero because this ablation intentionally isolates retrieval without an LLM.", "The independent verifier checks the business invariant in both arms, so verifier confirmation does not measure model discovery."]}
